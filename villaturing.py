...
class AplicacionTuring:
    def _init_(self, root):
        ...
        self.etiqueta = tk.Label(root, text="Ingrese exactamente dos números binarios separados por '+':")
        self.etiqueta.pack()
    ...
    def ejecutar_maquina_turing(self):
        ...
        if '+' not in cinta:
            messagebox.showerror("Error de Entrada", "La entrada debe contener exactamente dos números binarios separados por '+'.")
            return
        ...
        if len(partes_binarias) != 2 or not all(parte.isdigit() and set(parte).issubset({'0', '1'}) for parte in partes_binarias):
            messagebox.showerror("Error de Entrada", "La entrada debe contener exactamente dos números binarios (0 y 1) separados por '+'.")
            return

        # Agregar ceros a la izquierda para que ambos números binarios tengan la misma longitud
        longitud_maxima = max(len(parte) for parte in partes_binarias)
        cinta_rellenada = '+'.join(partes_binarias[0].zfill(longitud_maxima) + '+' + partes_binarias[1].zfill(longitud_maxima))
